...
namespace KitforgeLabs.UIKit.Editor.Generators
{
    internal static class CatalogGroupBuilderShared
    {
        ...
        private const string CatalogAsmdefName = "KitforgeLabs.UIKit.Catalog.asmdef";
        ...
        internal static string ResolvePrefabsFolder()
        {
            ...
            foreach (var guid in AssetDatabase.FindAssets("KitforgeLabs.UIKit.Catalog t:asmdef"))
            {
                ...
                if (!path.EndsWith($"/{CatalogAsmdefName}")) continue;
                var catalogDir = Path.GetDirectoryName(path).Replace('\\', '/');
                return $"{catalogDir}/Prefabs";
            }
            return UpmPrefabsFolder;
        }

        internal static void EnsurePrefabsFolder(string prefabsFolder)
        {
            if (AssetDatabase.IsValidFolder(prefabsFolder)) return;
