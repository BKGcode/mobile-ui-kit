...
Latest tag: `v0.9.0-alpha` (M4 hardening core packaging — 5 Catalog builders aligned via `CatalogGroupBuilderShared` + ThemedImage runtime contract + Theme presets validated end-to-end + QA Suite editor tool). Post-tag M4.X cluster in progress (audit tool unified at `Tools → Kitforge → UI Kit → Audit` via Path-A clean replacing legacy QA Suite + Stage 2 text-theming sweep B/C/D/E adding ~36 themed text callsites + `ThemedFieldsWiredCheck` audit lock + `Regenerate + Audit` one-click flow — see CHANGELOG `[Unreleased]`). M3c UX audit closed 2026-05-09 (4 blocks, 130 findings, 7 UX Pillars LOCKED, 7 Hub features driven, 4 critical decisions D1-D4 LOCKED — full audit at `~/.claude/memory/kitforge_ux_audit_2026-05.md`). Next: M5/RC implementation, **7 sub-milestones M5.1-M5.7** (Premises Runtime → Hub Editor window UIToolkit + Setup tab → Catalog Browser → Theme Studio → Test + Communication → HUDSimple + SafeAreaFitter built-in + Help → final triple gate + tag `v1.0.0-rc`).
...
```json
"com.kitforgelabs.mobile-ui-kit": "https://github.com/BKGcode/mobile-ui-kit.git#v0.9.0-alpha"
```
...
- **Catalog — M4.1 — Theme Presets** — `Theme_Casual.asset` (bright/saturated, Royal Match vibe) + `Theme_Premium.asset` (dark/desaturated, Hades vibe) + a `ThemeSwitcherSample` MonoBehaviour with a 3-option dropdown (Default / Casual / Premium) reskinning `GameOver` + `LevelComplete` popups. **Requires Group B + Group C samples** (Economy / Ads stubs + popup prefabs); the builder aborts with a clear LogError if either is missing. After importing, run `Tools → Kitforge → UI Kit → Build M4.1 — Theme Presets` to materialize the 2 themes (cloning Default's sprite/font/audio refs) + `ThemePresetsDemo.unity` under `Assets/Catalog_M4_ThemePresets_Demo/`. _M4 closed at `v0.9.0-alpha` — all 5 Catalog builders (A + B + C + D + E) consume `CatalogGroupBuilderShared` (6 constants + 17 helpers), ~30 themed callsites reskin uniformly across Default / Casual / Premium, QA Suite editor tool ships sequential structural integrity scanner (23 targets clean). M4.X cluster (post-tag, 2026-05-09) supersedes QA Suite with **UI Kit Audit** unified pipeline (`Tools → Kitforge → UI Kit → Audit`, Path-A clean — 7 pluggable `IUIKitCheck` impls via `TypeCache` including new `ThemedFieldsWiredCheck`, headless mode for CI, `Regenerate + Audit` one-click) + Stage 2 text-theming sweep adding 51 themed text counts to B/C/D/E. See [`CHANGELOG.md`](./CHANGELOG.md) `[0.9.0-alpha]` + `[Unreleased]` blocks._
