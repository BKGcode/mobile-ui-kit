...
Latest tag: `v0.8.0-alpha` BREAKING (Group E screens + OnUpdate infra dispatch — closes M3a). Next: `v0.9.0-alpha` (M4 hardening core packaging — Theme presets, severity icons, `CatalogBuilderBase` extraction, master demo, MIGRATION.md, perf bench, hero screenshots, fresh-import smoke test, API freeze) → M3c UX audit (no tag) → `v1.0.0-rc` (M5/RC final triple gate).
...
- **Catalog — M4.1 — Theme Presets** — `Theme_Casual.asset` (bright/saturated, Royal Match vibe) + `Theme_Premium.asset` (dark/desaturated, Hades vibe) + a `ThemeSwitcherSample` MonoBehaviour with a 3-option dropdown (Default / Casual / Premium) reskinning `GameOver` + `LevelComplete` popups. **Requires Group B + Group C samples** (Economy / Ads stubs + popup prefabs); the builder aborts with a clear LogError if either is missing. After importing, run `Tools → Kitforge → UI Kit → Build M4.1 — Theme Presets` to materialize the 2 themes (cloning Default's sprite/font/audio refs) + `ThemePresetsDemo.unity` under `Assets/Catalog_M4_ThemePresets_Demo/`. _M4 in flight — Group C reskin validated end-to-end via M4.7; Group A reconciled to canonical helper signature in M4.7-bis.A (2026-05-06) — both builders now aligned on `AddThemedImage(go, Color, ThemeSpriteSlot, ThemeColorSlot)` returns Image; Groups B / D / E sweep + `CatalogGroupBuilderBase` extraction pending M4.7-bis.B pre-tag `v0.9.0-alpha`. See [`CHANGELOG.md`](./CHANGELOG.md) `[0.9.0-alpha]` § Fixed + § Known issues._
