...
- **Catalog — M4.1 — Theme Presets** — `Theme_Casual.asset` (bright/saturated, Royal Match vibe) + `Theme_Premium.asset` (dark/desaturated, Hades vibe) + a `ThemeSwitcherSample` MonoBehaviour with a 3-option dropdown (Default / Casual / Premium) reskinning `GameOver` + `LevelComplete` popups. **Requires Group B + Group C samples** (Economy / Ads stubs + popup prefabs); the builder aborts with a clear LogError if either is missing. After importing, run `Tools → Kitforge → UI Kit → Build M4.1 — Theme Presets` to materialize the 2 themes (cloning Default's sprite/font/audio refs) + `ThemePresetsDemo.unity` under `Assets/Catalog_M4_ThemePresets_Demo/`. _M4 in flight — Group C reskin validated end-to-end via M4.7 (`ThemedImage` runtime read in `CatalogGroupCBuilder`); Groups A / B / D / E pending M4.7-bis sweep pre-tag `v0.9.0-alpha`. See [`CHANGELOG.md`](./CHANGELOG.md) `[0.9.0-alpha]` § Fixed + § Known issues._
