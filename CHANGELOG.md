...
- `GroupEDemoHost.BuildDailyLoginTemplate` now sets `CloseOnBackdrop = true` on the `DailyLoginPopupData` template. Restores by-design tap-out dismiss intent for the demo template (without the field set, `BackdropButton.interactable` stays false and the internal backdrop button consumes taps as no-ops). NOTE: this fix alone does NOT resolve the M3.1ter "MainMenu buttons unresponsive" symptom — see `### Known issues` below for the structural `PopupManager._backdrop` sibling-order bug that is the actual blocker. Setting `CloseOnBackdrop=true` is still correct because once the structural bug is fixed, the template behaves as the demo intends. Note: `DailyLoginFlow.ShowIfDue` is intentionally one-shot per launch (guarded by `_shownThisLaunch`) AND one-shot per UTC day (guarded by `IProgressionService.GetDailyLoginState().AlreadyClaimedToday`) — re-running `Boot Demo` after a successful claim will not re-spawn the popup. This is by-design kit behavior (DailyLogin is a launch-window prompt, not a recurring HUD) and is not a bug.

### Known issues

- **`PopupManager._backdrop` sibling-order bug — global backdrop renders ON TOP of active popups, blocking all input.** `PopupManager.ApplySiblingOrder()` reorders the active popup stack to indices `0..N-1` inside `_popupRoot` but does not touch the global `_backdrop` GameObject (created by builders via `CreateBackdropIn(popupRoot)` and assigned to `PopupManager._backdrop`). Because UGUI renders later siblings on top, the backdrop ends up ABOVE every popup with `raycastTarget=true` covering the full canvas — it intercepts every click intended for the popup itself (Claim, BackdropButton, Watch-Ad) and for any UI behind (MainMenu buttons in Group E, currency HUD in B/C demos). Symptom in Group E demo: DailyLogin auto-spawns over MainMenu, no button on either layer responds to clicks. Affects all groups B/C/D/E that ship a demo with a backdrop. **Workaround for buyers evaluating the kit at v0.8.0-alpha:** ignore the demo's interactivity for now — code paths and EditMode tests (292/292 green) confirm popup logic is correct in isolation; only the demo scene's input is broken. **Fix scheduled for M4 hardening:** `ApplySiblingOrder` will end with `_backdrop?.transform.SetAsFirstSibling()` so the backdrop stays at the bottom of `_popupRoot`. Verification will require re-running every group's Build-Sample → Press Play → first-button-click gate (per "Verify-all-samples gate" in `feedback_workflow.md`, refined 2026-05-06 to require button-click verification, not only visual render).
- **`v0.8.0-alpha` tag deferred.** This CHANGELOG block was authored at the M3 close-out gate but the tag was NOT cut: the Group E demo blocker above means buyers importing the sample at this commit would see broken interactivity on first Play. Phase C (commit + tag) is paused until the `PopupManager._backdrop` fix lands and the samples-green gate passes for all 5 groups. Until then, treat the work in this block as "in-flight on `main`" rather than tagged. Re-open this `[0.8.0-alpha]` header at the next session that closes M3.1ter; consider whether to keep the version label or bump to `0.8.1-alpha` to make the corrective milestone explicit.
