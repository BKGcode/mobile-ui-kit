...
- **Polish manifests in newly-built sample prefabs only — re-run `Tools/Kitforge/UI Kit/Build Group X Sample` after upgrade.** The animator code paths are inside the kit `Runtime/`, but the sample prefabs and demo scenes were generated by the previous-tag builders and reference the older animator state. Re-run the relevant Group B and Group C sample builders to regenerate prefabs against the polished animators. Buyers who fork the prefabs verbatim into their game must apply the same regeneration to their copies (or replace the animator component on the forked prefab).

### Documentation

- **`sealed UIAnim*Popup` clarified as canonical** in `Documentation~/Specs/CATALOG.md` § 3.1 MUST 4. The intent is binary extension surface: (a) override `UIAnimPreset` SO via `AnimPresetOverride`, or (b) replace the script entirely. Inheritance is intentionally blocked. Closes the spec contradiction tracked in roadmap line 55 (post-`/_checker` 2026-05-04 finding) — sealed was always the design, the spec wording was incomplete. No code change.
